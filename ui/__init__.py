...
class Blend4v_PT_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        subrow.prop(context.scene.blend4vcam, 'Filepath')
        subrow = layout.row(align=True)
        subrow.prop(context.scene.blend4vcam, 'Only Selected Camera')
        subrow = layout.row(align=True)
        subrow.prop(context.scene.blend4vcam, 'Multiple Files')
        subrow = layout.row(align=True)
        subrow.prop(context.scene.blend4vcam, 'To Text Block')
        subrow = layout.row(align=True)
